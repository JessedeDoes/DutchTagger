...
public class MultiClassifier implements Serializable
{
	...
	public void addModel(SVMLightModel model, String className)
	{
		if (className == null)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("NULL CLASS NAME " + labels.size());
			//System.exit(1);
			className = "NULL";
		}
		...
		if (weights == null)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("null weights for class " + className);
			return;
		}
	...
	public void noLists()
	{
		...
		for (Entry<Integer, ArrayList<Weight>> e: weightMap.entrySet())
		{
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(e.getKey() + "-->" + array);
	...
	public String classify(LabeledFeatureVector v)
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Classify:"  + labels.size());
		for (int i =0;  i < labels.size(); i++)
		{
			//double check = models.get(i).classify(v);
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("check" + i + "=" +check  + " "  + bestCheck + " " + bestLabelCheck + "?" + bestLabel + "?" +  	labels.get(i));
			
			//if (check != delta[i])
			//{
			  //org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(labels.get(i) + ":" + check + "!=" + delta[i]);
			//}
			if (delta[i] > best)
			{
				best = delta[i];
		...
		//{
		//	org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Oneenigheid! " + bestLabel + "!" + bestLabelCheck);
		//}
		return bestLabel;
	}

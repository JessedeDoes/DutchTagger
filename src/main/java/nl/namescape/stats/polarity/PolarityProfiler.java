...
public class PolarityProfiler implements org.ivdnt.openconvert.filehandling.DoSomethingWithFile
{
	int nTokens = 0;
	...
	public double getPolarity(String lemma)
	{
		...
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(lemma  + " + " + PLUS +  " - " + MINUS);
		return dPlus - dMin;
	}
	...
	boolean filter(Document d)
	{
		if (yearFrom != null && yearTo != null)
		{
			...
			try
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(wYearFrom + "--" + wYearTo);
	...
	public void handleFile(String fileName) 
	{
		try
		{
			...
			{
				if (yearFrom != null) org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("filter accepts " + fileName);
			}
			List<Element> sentences = nl.namescape.tei.TEITagClasses.getSentenceElements(d.getDocumentElement());
			if (sentences.size() == 0)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("no sentences, just using element text!");
			...
			for (Element s: sentences)
			{
				...
				for (int i=0; i < tokens.size(); i++)
				{
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(wordform + " " + lemma + " " + tag);
	...
	public class PolarityComparator implements Comparator<String> 
	{
		...
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(_base);
			this.base = _base;
		}

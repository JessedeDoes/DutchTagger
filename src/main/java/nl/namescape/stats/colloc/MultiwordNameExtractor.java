...
public class MultiwordNameExtractor implements DoSomethingWithFile
{
		...
		for (Element s: sentences)
		{
			...
			for (int i=0; i < tokens.size(); i++)
			{
						...
						indexOfFirstCapitalizedWord < maxLeadingLowerCaseWords+1) 
				{
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("looking in "  + nGram  + " minSize " + minSize + " maxSize " + maxSize);
					...
					for (int j=minSize; j <= nGram.size() && j <= maxSize; j++)
					{
						if (isCapitalized(nGram.get(j-1)))
						{
							...
							// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Consider: " + wng + " in " + Util.join(nGram, ", "));
						...
						indexOfFirstCapitalizedWord < maxLeadingLowerCaseWords+1) 
				{
					...
					for (int j=minSize; j <= nGram.size() && j <= maxSize; j++)
					{
						if (isCapitalized(nGram.get(j-1)))
						{
							...
							{
								//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("look at parts of " + wn);
								...
								for (int l=1; l < j; l++)
								{
									...
									if (!isCapitalized(wl) && l < j-2)
									{
										...
										//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("store right part: " + rightPart);
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("longest at i=" + i + ": " + lengthOfLongestStoredNGram);
					for (int j=3; j < lengthOfLongestStoredNGram; j++)
					{
						...
						if (!isCapitalized(wj))
						{
							...
							//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("store left part: " + leftPart);
				...
				for (int j=i; j < tokens.size(); j++)
				{
					...
					if (TEITagClasses.isWord(e))
					{
						...
						if (j -i > 1)
						{
							...
							// if (wn.toString().contains("Postma")) org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("count ngram: "  + wn);
							if (this.otherNgramCounter.containsKey(wn))
							{
								//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Seen again:" + wn);
								this.otherNgramCounter.increment(wn);
							} else
							{
								// if (wn.parts.get(0).equals("Dirk"))
								//	org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Nope " + wn);
							}
						}
					} else break;
				}
	...
	private void pruneNgrams() 
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("We have "  + ngramCounter.size() + " ngrams! ");
	}
	...
	private void scoreNgrams() 
	{
		...
		for (WordNGram wn: ngrams)
		{
			...
			double d = SCP(wn);
			wn.score = d;
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(f + " " + wn  +  " SCP: " + d);
		}
		...
		for (WordNGram wn: nGramList)
		{
			...
			{
				int f = ngramCounter.get(wn);
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(f + " " + wn  +  " SCP: " + wn.score);
			}
		}
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("We have "  + ngramCounter.size() + " ngrams! ");
	}
	...
	private Counter<String> getListOfPossibleNameFunctionWords()
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("LIST OF TOP LOWER CASE PARTS");
		for (String s: lowerCaseParts.keyList())
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(lowerCaseParts.get(s) + "\t" + s);
		}
	...
	{
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Stage " + stage + ": "  + fileName);
		try
		{
...
*/
	...
	private double SCP(WordNGram w)
	{
		...
		for (int i=1; i < n; i++)
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("!!Problem at i=" + i + " with " + w + " f1 = "  + f1 + " f2 = "  + f2);
			}
		}
	...
	private int getSpanFrequency(WordNGram w, int start, int end)
	{
		...
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Try as name " + wng);
			return this.ngramCounter.get(wng);
		}
		else
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Try as nonname " + wng);
			int f = this.otherNgramCounter.get(wng);
			if (f == 0)
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Huh... " + wng  + " in other Ngram hash?: " + otherNgramCounter.containsKey(wng));
			return f;
		}
	}
	...
	private void resetOtherNGramCounters() 
	{
		...
		{
			e.setValue(0);
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Whoops " + e);
		}
	}
	...
	public static void main(String[] args)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Processors: " + processors);

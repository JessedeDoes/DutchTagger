...
public class NameFrequencyList implements org.ivdnt.openconvert.filehandling.DoSomethingWithFile
{
	...
	{
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(fileName);
		try
		{
		...
		for (WordList.TypeFrequency x: tf.keyList(true))
		{
			...
			if (e != null)
			{
				...
				while (i.hasNext())
				{
					...
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(exNo + "\t" + idNo + "\t"  + sentence);
					exNo++;
				}
			}
			idNo++;

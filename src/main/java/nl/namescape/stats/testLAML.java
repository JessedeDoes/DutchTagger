	...
	public Matrix getLinearMappingBetweenSpaces(Vectors v1, Vectors v2)
	{
		...
		for (int i=0 ; i < v1.wordCount(); i++)
		{
			if ((!useP  ||  Math.random() < p) && (!useMax ||  k < max))
			{
				...
				if (v2.hasTerm(w)) try
				{
					// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("selected " + w);
					float[] vec2 = v2.getVector(w);
					if (vec2 != null)
					{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Selected + " + vectors1.size());
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Computed linear mapping in " +(f-s) + " milliseconds ");
		return lasso.W;
	}

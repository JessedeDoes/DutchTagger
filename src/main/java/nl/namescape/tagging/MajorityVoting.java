	...
	public void checkDifferentTaggingsOfSameName(Document d)
	{
		...
		for (String s: withoutType.keySet())
		{
			...
			if (f >= minFrequency)
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("looking at " + s + " top: " + topType);
				if (typeMap.get(topType) >= minProportion * f) // regard other types as nonsensical
				{
					for (Element e: withoutType.getInstances(s))
					{
						...
						{
							org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("VOTING: CHANGE TYPE OF " + s + " FROM "  + typeWas + " TO " + topType + " " + ec.getConcordance(e));

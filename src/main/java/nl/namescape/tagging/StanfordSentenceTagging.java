...
public class StanfordSentenceTagging
{
	...
	public static String tagSentence(String input, List<AbstractSequenceClassifier> list)
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("tagSentence (toplevel) Input:" + input);
		for (AbstractSequenceClassifier a: list)
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("intermediate now: " +  intermediate);
	...
	public static String tagSentence(String input, AbstractSequenceClassifier asc) 
	{
		String line="";
		int k=0;
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("tagSentence (step) Input:" + input);
		...
		{       	
			...
			{
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("CHECK FAILED: " + inputTokens[k] + " != " + x.word());
			}
			if (printTags)
	...
	public static List<CoreLabel> getTokenList(String sentence) // je kan vast wel altijd TaggedWord doen, maar ja...
	{
		...
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("token: <" + s + ">");
			if (s.contains("\t"))
			{
				String[] parts = s.split("\t");
				CoreLabel t = makeCoreLabel(parts[0], parts[1]);
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("TAG:"  + t.tag());
				tokens.add(t);
			} else
	...
	{
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("x.NER:" + x.ner());
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("position: " + x.beginPosition() + " ---> "  + x.endPosition());
		for (Class s: x.keySet())
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s.getClass() + "<"  + s);
			try
			{
				String v = x.getString(s);
	
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s + " --> " + v);
			} catch (Exception e)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("cannot get: " + s);
			}
		}
	}
}

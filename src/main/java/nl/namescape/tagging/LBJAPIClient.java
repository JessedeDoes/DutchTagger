...
public class LBJAPIClient 
{
	...
	public void tagXMLFileUsingLBJNerTaggerAPI(String inputFile)
	{
		...
		sw.stop();
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("document tokenized: " + sw.getElapsedTimeSecs());
		sw.start();
		Vector<LinkedVector> data= parseXML(d);
		sw.stop();
		
	
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("tokenized document parsed: " + sw.getElapsedTimeSecs());
		...
		sw.stop();
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("NER performed: " + sw.getElapsedTimeSecs());
	...
	public Vector<LinkedVector> parseXML(Document d)
	{
		...
		for (Element p: XML.getElementsByTagname(d.getDocumentElement(), "s", false))
		{
			...
			for (Element w: XML.getElementsByTagname(p, "w", false))
			{
				...
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("error parsing token " + token + " word: "  + word);
					e.printStackTrace();
					continue;
				}

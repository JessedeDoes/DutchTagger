...
import org.ivdnt.openconvert.filehandling.ConversionException;
	...
	public void tagDocument(Document d)
	{
		try 
		{
			...
			if (!tokenize)
			{
				...
				if (eek)
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("eek!!!");
			}
	...
	public void handleFile(String in, String out) throws ConversionException
	{
		...
		if (tokenize)
		{
			try
			{
				...
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("document has " + TEITagClasses.getNumberOfWords(d) + " words ");
			} catch (Exception e)
			{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("NOT tokenizing " + in);
			try 
			{
				d = XML.parse(in);
			} catch (Exception e) 

...
public class FrogClient extends DocumentTagger implements SentenceTagger
{
	...
	public String tagString(String sentence)
	{
		...
		try 
		{
			...
			{
				/// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("received" + line);
				answer.add(line);
				taggedSentence += line + "\n";
		...
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(String.format("%x",x) + ":" + sentence);
	...
	private String  checkAlignment(String[] inputTokens, List<String> answer) 
	{
		...
		for (int i=0; i < inputTokens.length; i++)
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(tw.get(i).token + "?" + inputTokens[i]);
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("problem in sentence: at " + i + "\n" + util.Util.join(answer, "\n") + "\n\n");
				allOk=false;
				break;
			}
		}
		...
		if (!allOk)
		{
			...
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("trying to fix....");
			...
			{ 
				...
				{
					canBeFixed=false;
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("failure [tw te groot]  while fixing!!!!");
					break;
				}
				...
				} else
				{
					...
					{
						org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("failure while fixing!!!! " + i +  " " + lookFor + " " + k);
						canBeFixed=false;
			...
			if (canBeFixed)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("sentence could be fixed");
				fixedResult="";
				for (int i=0; i < inputTokens.length; i++)
				{
					...
					// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(i + " " + tok + " " + tag);
				}
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("FIXED:" + fixedResult);
				return fixedResult;
			}
		}
		return null;
	...
	public void connect()
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Unknown host: " + server);
			System.exit(1);
		} catch  (IOException e) 
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("No I/O: " + e);
			//System.exit(1);
		}
	}

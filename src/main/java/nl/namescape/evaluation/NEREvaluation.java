...
public class NEREvaluation 
{
	...
	public void evaluate(NETaggedDocument truth, NETaggedDocument tagged)
	{
		...
		for (Entity e: trueEntities)
		{
			...
			if (e1 != null)
			{
				if (verbose) org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Found " + e1);
			...
			{
				if (verbose) org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Missed: " + e +  " at " + location);
		...
		for (Entity e: taggedEntities)
		{
			...
			{
				if (verbose) org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("False positive: " + e +  " at " + location);

 ...
 *
 */
public class TEINameTagging implements org.ivdnt.openconvert.filehandling.SimpleInputOutputProcess
{
	...
	private static void tagStructureAttribute(Element e) 
	{
		if (e.getAttribute("type").equals("person"))
		{
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(parts.size() + " in " + XML.NodeToString(e));
			if (parts != null)
			{
				for (Element p: parts)
	...
	{
		org.ivdnt.openconvert.filehandling.DirectoryHandling.tagAllFilesInDirectory(new TEINameTagging(), args[0], 
				args[1]);

...
public class Validator implements DoSomethingWithFile
{
	...
	public Validator(String url)
	{
		try
		{
			...
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("created validator from " + url + " : "  + thaiValidator);
		} catch (Exception e)
		{
			e.printStackTrace();
		}
	...
	public boolean validate(Document d)
	{
		...
		try 
		{
			...
			{
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Errors: " + errorCounter.getErrorCount());
				valid = false;
			} else
			{
				valid = true;

...
import org.ivdnt.openconvert.filehandling.*;
...
public class MolechaserMetadataChecker implements DoSomethingWithFile
{
	...
	public void handleFile(String fileName) 
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(N + ": "+ fileName);
		try
		{
	...
	public synchronized void checkMetadata(Metadata m, Document d, String fileName)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("No corpusProvenance in file:  " + fileName);
		}
		
		if (!isYear(witnessYear_from))
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("invalid year in " + fileName + " : " + witnessYear_from);
		}
		if (!isYear(witnessYear_to))
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("invalid year in " + fileName + " : " + witnessYear_to);
		}
		...
		if (checkIdno)
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("No idno in file:  " + fileName);
				String newIdno = MolechaserMetadataFixer.createIdno(m, d, fileName);
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Attempt to assign idno: " + newIdno);
				sourceId = newIdno;
			}
			...
			} else
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Error: duplicate idno "  + idno + " " + filesWithThisId);
			}
			filesWithThisId.add(fileName);
		}
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Wrong number of language variants: " + languages + ":  " + fileName);
		} else
		{
			l = languages.iterator().next();
			if (!l.equals("NN") && !l.equals("BN"))
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Wrong language variant: " + l +  ":  " + fileName);
			}
		}
		...
		for (String name: m.metadata.keySet())
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Multiply defined property " + name +  " in " + fileName +  " :  " + values);
		...
		if (checkDuplicateArticles)
		{
			...
			} else
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Error: duplicate content? "  +key + " " + filesWithThisKey);
			}
			filesWithThisKey.add(fileName);
		}
	...
	public synchronized void print(Metadata m, String fileName)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(N + "\t" + corpusProvenance+ "\t" +  fileName + "\t" + languageVariant + "\t" 
				+ author + "\t" + title + "\t" + 

...
public class Metadata 
{
	...
	public static Map<String,Set<String>> getMetadata(Document d)
	{
		...
		for (Element b: bibls)
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(b);
			List<Element> grps = XML.getElementsByTagname(b, "interpGrp", false);
			for (Element grp: grps)
			{
				String fieldName = grp.getAttribute("type");
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("interpGrp type=" + fieldName);
				...
				for (Element i: intrps)
				{
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(fieldName + ":"  + content);

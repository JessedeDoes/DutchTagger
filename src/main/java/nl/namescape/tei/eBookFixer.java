...
public class eBookFixer implements DoSomethingWithFile
{
			...
			for (int i=0; i < ch.getLength(); i++)
			{
				...
				if (c.getNodeType() == Node.ELEMENT_NODE)
				{
					...
					if (tag.equals("div") || tag.equals("p") || tag.equals("head"))
					{
						if (wrapping)
						{
							org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Stop wrapping at "  + c);
					...
					} else // no reason to stop wrapping...
					{
						...
						{
							wrapping = true;
							org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Start wrapping at "  + c);
	...
	private void wrapRange(Document d, Node firstInRange, Node lastInRange, String elementName)
	{
		...
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(e.getParentNode() + " " + e.getTextContent());
		range.detach();
	}
	...
	public void handleFile(String fileName) 
	{
		...
		try
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("try to fix " + fileName);
				fixDocument(d);
				if (validator.validate(d))
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Opgelapt!!!" + fileName);
				}
			}
		} catch (Exception e)
		{

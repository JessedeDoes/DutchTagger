...
public class ObjectRelationalMapping
{
	...
	public List<String> getPrimaryKeys(Connection connection, String tableName)
	{
		...
		try
		{
			...
			while (rs.next()) 
			{
				...
				list.add(columnName);
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("getPrimaryKeys(): columnName=" + columnName);
			}
			rs.close();
		} catch (Exception e)
		{
	...
	public void setPrimaryKeyField(String fieldName)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(javaClass.getName() + " does not have a field "  + fieldName);
			e.printStackTrace();
		}
	}
	...
	public Object getNextAsObject(ResultSet rs)
	{
		try
		{
			if (rs.next())
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("meta = " + rs.getMetaData().getColumnCount());
				for (int i=1; i < rs.getMetaData().getColumnCount(); i++)
				{
					String dbFieldName = rs.getMetaData().getColumnName(i);
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(i);

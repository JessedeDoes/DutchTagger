...
public class TEITokenStream implements TokenWindow, Corpus
{
	...
	private boolean tagSentencesInElement(Node n) 
	{
		...
		if (tokenElements.size() > 0)
		{
			...
			for (Element e: tokenElements)
			{
				...
				if (t.getIsEOS() && !breakForbidden(e))
				{
					...
					if (nSentences % 100 == 0)
					{
						...
						//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("sentence splitting:... "  + nSentences + " : " + sentence.replaceAll("\\s+"," "));
					}
					if (s != null)
			...
			if (nWrappedWords < k)
			{
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("!!problem in (nW=" +  nWrappedWords + ",k= " + k   + "): " + XML.NodeToString(n));
				unwrappedWords = true;
			}
			k=0;
		...
		for (int k=0; k < children.size(); k++)
		{
			...
			} else
			{
				if (inWordGroup)
				{
					...
					if (s != null)
						org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("created sentence chunk: " + XML.NodeToString(s));
					inWordGroup = false;
				}
				wrapStrayWordsIn(c);
			}
	...
	private Element makeSentenceElement(List<Element> tokzInP, int sStart, int k)
	{
		try
		{
			...
			} else // try to fix by moving up...
			{
				...
				{
					/*
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("HOERA: "  + XML.NodeToString(startNode) + " -- "
				...
				} else
				{
					...
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("AHOOPS: failed to tag sentence from "  + word1 + " to "  + word2);
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("common ancestor: " + commonAncestor);
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Warning: failed to tag sentence because of hierarchy conflict");
		}
		return null;
	}
	...
	private Element createSentenceFromTo(Element p1, Element e1, Element e2) 
	{
		...
		{
			ex.printStackTrace();
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Problem: s=" + XML.NodeToString(s));
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("p1 = " + XML.NodeToString(p1));
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("e1 = " + XML.NodeToString(e1));
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("e2 = " + XML.NodeToString(e2));
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("after= " + after  +  " -- " + after.getParentNode());
		}
		return s;
	}

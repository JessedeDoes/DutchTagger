...
public class FeatureSet implements Iterable<Feature>, Serializable
{
	...
	public void pruneInstance(Instance instance)
	{
		for (int i=0; i < features.size(); i++) // never prune class feature
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("NOPE");
				System.exit(1);
			}
			...
			if ((c = f.valueCounts.get(v)) == null || c < f.THRESHOLD)
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(this.itemToString(instance));
			} else
			{
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Ok, dit is genoeg: " + c);
			}
		}
	}
	...
	public Instance makeTestInstance(Object s)
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("fs: " + features.size());
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(item);
		return instance;
	}
	/**

...
public class WekaClassifier implements Classifier
{
	...
	void createInstances(Iterable<Instance> instances)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(fi + " " + fi.values);
			if (fi == features.classFeature && fi.values.size() ==1) // er moet meer dan een mogelijke waarde voor class zijn!
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Voeg dummy waarde toe voor " + name);
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(instances);
	}

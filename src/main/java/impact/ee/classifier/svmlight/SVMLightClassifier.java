...
public class SVMLightClassifier implements Classifier, Serializable
{
		...
		try 
		{
			...
			for (File mf : modelFiles) 
			{
				...
				try 
				{
					...
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("class label = " + d  + ", class name = " + className + " file = " + mf.getCanonicalPath());
					if (useMultiClassifier)
					{
			...
			TrainingParameters tp, String targetLabel, String otherLabel, double targetClass) 
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Training SVM-light model for " + targetLabel + " " + otherLabel);
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Training done.....");
	}
	...
	public void validateModel(SVMLightModel model, Problem p, double target)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("errors for " + inverseLabelMap.get(target) + " = " + errors);
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("fitting: "  + (p.size() - errors) / (double) p.size());
	}
		...
		for (String l1: this.labelMap.keySet())
		{
			for (String l2: this.labelMap.keySet())
			{
				if (l1.compareTo(l2) > 0)
				{
					...
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("problem size " + p1.size());

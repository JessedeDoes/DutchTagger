			...
			new Thread(new Runnable() 
			{
				...
				public void run() 
				{
					try 
					{
						...
						{
							if (verbose)
								org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("[Stdout] " + line);
							// als regel "examples read " bevat, kan
							// je de trainingFile weggooien...
							if (line.contains("examples read"))
							{
								org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("deleting combined training file...");
								tf.delete();
							}
						}
						brCleanUp.close();

...
public class SuffixGuesser implements Classifier
{
	...
	void addWordToSuffixTrie(String w, String cls)
	{
		...
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(w + ":  " + cls + " inserted: " +  i);
	}
	...
	public void computeSuffixDistributions()
	{
		Trie.NodeAction action = new Trie.NodeAction()
		{
			...
			{ 
				...
				{
					// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("no distribution defined at node: " + n);
				}
			}
		};
		suffixTrie.forAllNodesPreOrder(action);
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("trie has " + action.N + " nodes");
	}
	...
	private void computeThetas()
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("theta0 =  " + theta0);
		//System.exit(0);
		for (int i=0; i <=  M; i++)
			theta[i] = theta0;
	}
	...
	public Distribution distributionForString(String s)
	{
		...
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s + " match suffix length:  " + i);
		...
		for (int j=1; j  < M && path[j] != null;  j++)
		{
			...
			if (dnext == null)
			{
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("?! No distribution defined for  " + s + " at " + j + " node = "  + path[j]);
			}
			else
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(d);
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("s= " + s + " j= " + j + " d= " + d);
		}

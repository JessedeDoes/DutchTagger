...
public class ReverseLemmatizationTest implements FoundFormHandler
{
	...
	public void foundForm(String lemma, String tag, String lemmaPoS, Rule r, double p, int rank)
	{
		...
		if (wf != null)
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("+: " + wf + ": " + lemma  + ":" + tag + ":"  + r);
				correctProposals++;
			} else
			{
				incorrectProposals++;
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Could not apply to " + lemma + ": "  + r);
		}
	}
	...
	public void runTest(ParadigmExpander r)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Created held-out set of size " + heldOut.size());
		r.findInflectionPatterns(all, heldOut);
		for (WordForm wf: heldOut) r.expandWordForm(wf);
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Total tested: " +  itemsTested + 

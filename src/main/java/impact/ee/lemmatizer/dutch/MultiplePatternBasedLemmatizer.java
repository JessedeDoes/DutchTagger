...
public class MultiplePatternBasedLemmatizer extends SimplePatternBasedLemmatizer 
{
	...
	public void train(InMemoryLexicon lexicon, Set<WordForm> heldOutSet)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Words " + nWords + "  fallbacks " + nFallbacks);
		...
		for (WordForm w: lexicon) 
		{
			...
			if (w.tag.matches("VRB.*part.*past.*") && w.lemma.matches(".*en$"))
			{
				if (!w.wordform.matches(".*e$"))
				{
					...
					additions.add(w2);
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Is dit de w2 die je zoekt??? " + w2);
				}
			}  
		}
	...
	protected String findLemmaConsistentWithTag(String wordform, String corpusTag) 
	{
		...
		{			
			...
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Try classifier for " + lexiconTag + " for " + wordform +  ", corpus tag="  + corpusTag);
	...
	private String checkLexiconAndCache(String wordform, String corpusTag) 
	{
		...
		} else
		{
			...
			if (lemmata != null)
			{
				...
				{
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Word in lexicon, maar niet met passende tag.. " + wordform + ":" + corpusTag + " " + lemmata);
				}
			} 
		}
		return lemma;
	...
	protected void testWordform(WordForm wf, TestDutchLemmatizer t)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Error: " + wf + " --> " + lemma + "  " + lastRule);
		}
		t.nItems++;
	}
	...
	public HashMap<String, String> apply(Context c) 
	{
		...
		if (findMultiple)
		{
			...
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Nothing logged for " + word +  "/" + tag);
			}
		}
	...
	public void testOnLemmatizedCorpus(String corpusFileName)
	{
		...
		for (Context c: testCorpus.enumerate())
		{
			...
			if (teltMee)
			{
				...
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Dit kan niet " + wordform + "/"  + trueLemma + " assigned lemma=" + assignedLemma + " lemmata = "  + assignedLemmata);
				}
			...
			if (teltMee || mismatch)
			{
				...
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("No log information for " + wordform + ":" + corpusTag);
					//System.exit(1);
				}
			}
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("items " + nItems +  " errors "  + nErrors + " heeft correct  " + nHasCorrect +  " is correct " + (nItems - nErrors));
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("error percentage: " + (nErrors) / (double) nItems);
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("recall percentage: " + (nHasCorrect) / (double) nItems);
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("unrecalled content words: " + nNotRecalledContentWords);
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("recall percentage for content words: " + 
				(nContentWords - nNotRecalledContentWords) / (double) nContentWords);
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(lemmaLog.getGuesserPoSMatches());
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(errorTypeCounter);
	}

...
public class SimplePatternBasedLemmatizer implements java.io.Serializable, Tagger
{
	...
	public void train(InMemoryLexicon lexicon, Set<WordForm> heldOutSet)
	{
		...
		try
		{
			for (WordForm w: lexicon) // volgorde: type lemma pos lemma_pos /// why no ID's? it is better to keep them
			{
				...
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(w + " " + rule);
	...
	public void test(impact.ee.lexicon.InMemoryLexicon l)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(testResults);
	}
	...
	protected void checkResult(WordForm wf, String answer, Distribution outcomes, TestDutchLemmatizer t) 
	{
		...
		if (r == null)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("HUH?: " + answer);
		} else if (true || !wf.wordform.equals(wf.lemma))
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(r);
			String guessedLemma = r.pattern.apply(wf.wordform);
			if (guessedLemma == null)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Dit kan dus niet:.... " + r);
				return;
			}
			boolean isOK = guessedLemma.equalsIgnoreCase(wf.lemma);
			if (isOK) t.nCorrect++;
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("First choice: (" + isOK + ") " + wf + " (" +  r +  ") " + r.PoS + " : " + wf.wordform + " --> " + guessedLemma);
			if (guessedLemma == null || !guessedLemma.equals(wf.lemma))
			{
				...
				for (Outcome o: outcomes.outcomes)
				{
					...
					if (r1 == null)
					{
						org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Vreemd hoor!!!" + o.label);
						continue;
					}
					...
					{
						foundTagMatch = true;
						org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("\tguess with complete tag information: ("  + okNOW + ") " + guess +  " " +  r1);
						if (okNOW)
							t.nCorrectGivenTag++;
					} 
					...
					{
						foundPoSMatch = true;
						org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("\tguess with main PoS information: ("  + okNOW + ") " + guess +  " " + r1);
						if (okNOW)
							t.nCorrectGivenPoS++;
					}
				}
	...
	public static void main(String[] args)
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(l.findLemmata("is"));
		//System.exit(1);

...
public class DutchPatternFinder implements PatternFinder 
{
	...
	public Pattern findPattern(String a, String b, String PoS) //  a is word form, b is lemma in applications
	{
		...
		for (String suffixa: suffixes)
		{
			for (String suffixb: lemmaSuffixes)
			{
				if (a.endsWith(suffixa) && b.endsWith(suffixb))
				{
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(stemA + " " + stemB);
					for (String infix: infixes)
					{
						for (String stripped: StringUtils.removeInfix(stemA, infix))
						{
							...
							{
								//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(stemA + " " + stemB + "?"+ change);
								...
								if (x != null && x.equals(stemB))
								{
									...
									P.add(p);
									//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(p);
									//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(stripped + "-" + suffixa + " -->" + stemB + "-"  + suffixb + " : " + change.type);
								}
							}
						}
					}
		...
		if (!found)
		{
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Fallback to default for " + a + "~" + b +   " : " + foundPattern);
	    ...
	    {
	    	int k=0;
	    	org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("multiple for " + a + "~" + b);
	    	for (DutchPattern p: P)
	    	{
	    		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(k++ + ":" + p);
	    	}
	    }
		return foundPattern;
	}
	...
	public static void main(String[] args)
	{
				...
				} else
				{
					...
					} else
					{
						...
						if (d.stemChange.equals(RegularStemChange.IRREGULAR_STEM_CHANGE))
							org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(d);
						explained++;
					}
				}
			}
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Explained: " + explained  + " unexplained: " + unexplained);
	}
}

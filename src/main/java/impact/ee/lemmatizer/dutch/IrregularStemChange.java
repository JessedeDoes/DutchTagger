...
public class IrregularStemChange extends StemChange 
{
	...
	public void setData(String[][] data)
	{
		...
		for (String s:  baseForms.keySet())
		{
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(rev + "-->"  + baseForms.get(s));
	...
	public void test()
	{
		for (String[] line: data)
		{
			...
			for (int i=1; i < line.length; i++)
			{
				...
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(line[i] + "->" + test +  " truth:  "+ line[0]);
			}
		}
	}
	...
	public String transform(String s, Set<String> result, boolean multi)
	{
		...
		for (i=0; i < s.length() && node != null; i++)
		{
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(i + ":" + deepestNode);
			node = node.delta(s.charAt(s.length()-i-1));
		}
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s + ":" + suffixLength);
		...
		if (deepestNode != null)
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(i +  " " + s + deepestNode.data);
			...
			if (candidates != null)
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s + " suffix:"  + suffix);
				return s.replaceAll(suffix + "$", replacement);
			} else
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("DIT KAN DUS NIET" + s);
			}
		}
		return null;
	}

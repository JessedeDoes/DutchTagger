...
public class UnigramTransducer implements java.io.Serializable, CodeToStringPairMapping
{
	...
	public double jointProbability(Alphabet.CodedString x, Alphabet.CodedString y)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Fatal error: Joint probability undefined!");
			System.exit(1);
		}
		if (AlphaMN < 0)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Warning: Joint probability < 0 (" + x + ", " + y + ")");
		}
		return AlphaMN;
	}
  ...
  private double getNewAlpha(int s, int t, double α)
  {
	 ...
	 if (Double.isNaN(r))
	 {
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Error: Bad delta value for:" + s + ","  + t + 
	...
	public void estimateParameters(Dataset d)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Iteration: round " + i);
	...
	private void expectationMaximization(Dataset d)
	{
		this.dataset = d;
		int m = d.size();
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("piep " + m);
		...
		for (int i=0; i < m; i++)
		{
			...
			if (!pickedBestMatch)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("no best match picked for:" + dataset.items.get(i).target);
			}
		}
	}
			...
			double [][] Γ, double Γ_stop, double lambda, double scale) // stop_d was reference!
	{
		...
		{
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("alpha[T][V] zero after forward evaluation!!\n");
			return Γ_stop;
		}
		...
		for (int t=0; t <= T; t++)
		{
			for (int v=0; v <= V; v++)
			{
				...
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("norm boem " + norm +  " "  + alpha[T][V]);
					System.exit(1);
				}
		...
		if (debug)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Dump gamma:\n");
	...
	private void maximizationStep(double[][] Γ, double Γ_stop) // stop_d was reference
	{
		...
		for (int i=0; i < nSymbolPairs; i++)
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("boem " + N);
				System.exit(1);
			}
		}
	...
	{ 
		...
		for (int t=0; t < T+1; t++)
		{
			for (int v=0; v < V+1; v++)
			{
				...
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.printf("Error: bad alpha value computed:... %d %d %s = %s\n", t, v, 
							x, y);
					System.exit(1);
				}
			}
		...
		if (Double.isNaN(α[T][V]))
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Fatal error: unstable forward evaluation; dump α matrix\n");
	...
	private void init()
	{
		...
		for (int i=0; i < nSymbolPairs; i++)
		{
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.printf("%d: d('%d','%d')=%f\n",i,E[i].a,E[i].b,d);
			delta[e.lhs][e.rhs] = d;
	...
	protected int getCodeForSymbolPair(int i, int j)
	{
		...
		if (i != i1 || j != j1)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("inconsistency!");
			System.exit(1);
		}
	...
	void dumpParameterTable(String filename)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("symbolpairs: " + nSymbolPairs + " N= "  + N);
		try
		{
	...
	private void dumpParameters(double [][]x, int m, int n)
	{
		for (int i=0; i < m; i++)
		{
			for (int j=0; j < n; j++)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.print(" " +  x[i][j] + " ");
			}
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.print("\n");
		}
	}
	...
	public static void main(String [] argv)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Data read: " + d.size() + " items");

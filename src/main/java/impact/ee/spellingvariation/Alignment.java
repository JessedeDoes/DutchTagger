...
public class Alignment
{
	...
	public AlignmentGraph constructRestrictedGraph()
	{
		...
		for (GraphPath<Position,Transition> p: pathList)
		{
			...
			for (Object o: p.getEdgeList())
			{
				...
				if (e == null)
				{
					   /* org.ivdnt.openconvert.log.ConverterLog.defaultLog.printf("Add edge from [%d,%d] to [%d,%d] symbol [%d=%c/%c] weight=%f\n",
			...
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(p.getEdgeList().size() + " " +  p.getWeight());
		}
	...
	{  
		...
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(x + ", " + y + ": " + pathList.get(0).getWeight() + "-" + pathList.get(pathList.size()-1).getWeight());
	...
	{
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Full graph has " 
					+ fullGraph.vertexSet().size() + " vertices and "
					+ fullGraph.edgeSet().size() + " edges.");
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Restricted graph has " 

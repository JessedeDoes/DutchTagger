...
public class MultigramTransducer implements java.io.Serializable
{
  ...
  int nMultigrams()
  {
	...
	public synchronized void estimateParameters(Dataset d)
	{
		...
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("order=" + multigramSet.order);
		...
		for (int i=0; i < this.dataset.size(); i++)
		{
			...
			for (Candidate cand: item.candidates)
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Item " + i + ": " + cand.wordform + ": segmentation graph has  " 	+ cand.segmentationGraph.edgeSet().size() +  " egdes");
			}
		}
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Held out from " + heldOutFrom);
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Iteration " + i);
			double ll = this.expectationMaximization(this.dataset);
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Log likelihood now: " + ll);
		...
		for (Dataitem item: dataset.items)
		{
			...
			for (AlignmentSegmenter.Transition t: g.edgeSet())
			{
				...
				if (m.groupId >= 0)
				{
					...
					// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("abstracting away: " + m + " for " + a + " :  " + a.id);
					...
					// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(multigramSet.getScore(a.id));
				}
			};
		}
	}
	...
	{
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Printing multigrams:");
	...
	public void printAlignments(PrintWriter out) 
	{
		for (Dataitem item: dataset.items)
		{
			...
			for (AlignmentSegmenter.Transition t: g.edgeSet())
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("weight for "  + m +  " = " + d);
	...
	private void initializeFromSingletonTransducer(UnigramTransducer t, Dataset dataset)
	{
		...
		for (int i=0; i < dataset.size(); i++)
		{
			...
			for (Candidate c: item.candidates)
			{
				...
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Item " + i + ": " + c.wordform + ": alignment graph has  " 	+ c.alignmentGraph.edgeSet().size() +  " egdes");
 ...
 */
	...
	private void countMultigramsFromTo(AlignmentGraph g, Position from, Position to, int p, double lambda) 
	{
		...
		for (Alignment.Transition t: g.outgoingEdgesOf(to))
		{
			...
			if (p >= 0)
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("at Position " + to + "; at node: " + n);
				for (int i=0; i <= p; i++) 
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.printf("alpha=%e beta=%e alphaEnd=%e delt=%e\n", alpha,beta,alphaEnd,delt);
				w = alpha * beta  * delt;
				w = lambda * w / alphaEnd;
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("total added weight: " + w + "\n");
	...
	private double jointProbability(SegmentationGraph gxy)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Fatal error: Joint probabilty undefined!");
			System.exit(1);
		}
		if (alphaEnd < 0)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Warning: Joint probability < 0");
		}
		return alphaEnd;
	}
	...
	private double expectationMaximization(Dataset d)
	{
		this.dataset = d;
		int m = d.size();
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("beep " + m);
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Expectation steps...");
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Held out likelihood " + heldoutLogLikelihood);
			...
			double lambda, double scale)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("alpha ends on zero after forward evaluation !! " + alphaEnd);
			return Γ_stop;
		}
		...
		while (i.hasNext())
		{
			...
			for (AlignmentSegmenter.Transition t: gxy.incomingEdgesOf(p))
			{
				...
				{
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("whoopsie");
				}
			}
		}
		return Γ_stop;
	...
	{
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Maximization step");
		double N =  Γ_stop;
		...
		for (int i=0; i < nMultigrams(); i++)
		{
			if (delta[i] == 0 && ! (Γ[i] == 0))
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("this cannot be happening " + i  + " " +multigramSet.getMultigramById(i) );
				System.exit(1);
			}
			if (Double.isNaN(Γ[i]) || Double.isInfinite(Γ[i]))
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Oh nee ..." + Γ[i] +  " in " + multigramSet.getMultigramById(i));
			}
			N += Γ[i];
		}
		...
		for (int i=0; i < nMultigrams(); i++)
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Fatal: cannot set delta[" + i + "]: N = " + N + " Γ = " + Γ[i]);
				System.exit(1);
			}
		}
	...
	public static void main(String [] argv)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("MultigramTransducer --  data read: " + d.size() + " items");

...
public class MatcherWithMemory<State> extends MemorylessMatcher
{
		...
		public void relaxEdges(ShortestPath sp, MatchState<BasicState> source)
		{
			...
			for (Segment segment: segmentation.segments[position])
			{
				...
				for (JointMultigram m: segment.multigrams)
				{
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(position + " " + this.state + "->" + next + " " + m);
					if (next != null)
					{
	...
	static class Segmentation
	{
				...
				TrieNode rhsNode)
		{
			if (rhsNode.isFinal)
			{
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("segment:"
		...
		for (int i = 0; i <= s.length(); i++)
		{
			...
			for (Segment segment : segmentation.segments[i])
			{
				...
				if (false && loopSegment != null)
				{
					...
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("loop states at " + i + " : " + (newStates.size() - nonLoop));
				}
			}
			for (ScoredState<State> ss : newStates)
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(i +  ": " + ((History.State) (ss.state)).toString(multigramSet));
	...
	public static void testUnsmoothedModel(String modelFile, java.io.BufferedReader inFile)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("model read...");

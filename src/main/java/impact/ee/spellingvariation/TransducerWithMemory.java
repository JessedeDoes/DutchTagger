...
public class TransducerWithMemory implements java.io.Serializable
{
	...
	private Vector<Vector<ScoredState>> forwardEvaluation(SegmentationGraph g)
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("bloep");
		...
		for (int i=0; i < forwardVector.size(); i++)
		{
			...
			for (AlignmentSegmenter.Transition t: g.incomingEdgesOf(p))
			{
				...
				for (ScoredState sNs: previousStates)
				{
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(previousState +  "->" + nextState + " (" + t.multigramId + ")");
					if (nextState != null)
					{
						double w = conditionalP(previousState, t.multigramId);
						//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("forward(" + i + "): " + previousState +  "->" + nextState + " (" + t.multigramId + "," + w + ")");
						double deltaP = 0;
						if ( (deltaP = w * sNs.score) > 0)
						{
							...
							if (scoredStateMap[nextState.index]==null)
							{
								...
								graphSize++;
								//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("add state at " + i + ": " + nextState);
						...
						{
							//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("no fun for " + sNs.state + ": " + sNs.score);
						}
					}
				}
			}
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("graph size="  + graphSize);
		return stateLists;
	}
																						...
																						Vector<Vector<ScoredState>> alphas)
	{
		...
		// ToDo initialization..
		// history class should take care of starting up histories initially etc.
		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(forwardVector.size());
		...
		for (int i = forwardVector.size()-1; i >= 0; i--)
		{
			...
			for (AlignmentSegmenter.Transition t: g.outgoingEdgesOf(p))
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(i + "->" + nextNode.index); // of course...
				...
				for (ScoredState sNs: alphaStates)
				{
					...
					if (nextState != null)
					{
						...
						if (nextScoredState != null)
						{
							...
							//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("backward:(" + i + ") "+ 
		...
		for (Vector<ScoredState> a: alpha)
		{
			for (ScoredState s: a)
			{
				for (ScoredState.Link l: s.links)
				{
					...
					if (l.next != null)
					{
						//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("beep...");
					} else
					{
						org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("more or less killing state "  + s.state);
					}
				}
			}
		}
	...
	protected void printBestAlignment(SegmentationGraph g)
	{
		try
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Could not compute shortest path!");
			}
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	private void addToEvidence(History.State state, int multigramId, double e)
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("there is evidence "  + e + "  at " + state + "  for " + 
	  	 //this.memorylessTransducer.multigramSet.getMultigramById(multigramId));
		if (Double.isNaN(e) || Double.isInfinite(e) || e == 0)
		{
			// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Bah in addToEvidence" + e  + " state:" +  state);
		}
	...
	public void estimateParameters()
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Start building history transition diagram .. ");
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Initialize parameters from memoryless model ..");
		...
		for (History.State s: histories) // this should include the start state!
		{
			...
			for (History.Transition t: s.getStateTransitions())
			{
			  ...
			  // org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s +  "->" + t.target + " (" + t.symbol + "="  + t.p + ")");
			}
		}
		
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Start expectation mazimization .. ");
		for (int i=0; i < MAX_ITERATIONS; i++)
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(this.getClass().getName() + ": iteration " + i);
	...
	private void expectationMaximization()
	{
		...
		{
			if (k % 500 == 0)
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(this.getClass().getName() + ": Start expectation steps for items from " + k);
			for (Candidate c: item.candidates) expectationStep(c.segmentationGraph, c.lambda);
			if (k == (9 * dataset.size()) / 10)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("start collecting partial evidence " + k);
		...
		for (History.State s: histories)
		{
			...
			for (History.Transition t: s.getStateTransitions())
			{
				...
				{
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("never seen before:" + s + this.getMultigramSet().getMultigramById(t.symbol));
					unseenEvidence += t.evidence;
				} else
				{
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Yep seen before:" + s + this.getMultigramSet().getMultigramById(t.symbol));
				}
			...
			{
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Bah in maximization step: denominator = " + denominator + " in state " + s);
				continue;
			}
			
			for (History.Transition t: s.getStateTransitions())
				t.p = t.evidence / denominator;
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("set probability: " + s +  "->" + t.target + " (" + t.symbol + "="  + t.p + ")");
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Smoothing info: % unseen evidence: " + (unseenEvidence / totalHeldoutEvidence) + " .. " + totalHeldoutEvidence );
		...
		for (Dataitem item: dataset)
		{
			...
			for (Candidate cand: item.candidates)
			{
				...
				norm += cand.lambda;			
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("lambda = " + cand.lambda);
			}
			//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("norm = " + norm);
			if (item.heldOut)
			...
			if (!pickedBestMatch)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("no best match picked for:" + item.target);
			}
		}
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(this.getClass().getName() + ": Log likelihood now:" + logLikelihood);
	}
	...
	public static void main(String[] args)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Transducer with Memory: data read: " + d.size() + " items");

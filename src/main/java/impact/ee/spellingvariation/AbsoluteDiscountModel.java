  ...
  public State delta(State state, int c)
  {
	...
	public State delta(State state, int c, boolean tryUplink)
	{
		...
		} else
		{
			...
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("backing off: " + getEnclosingModelOrder(state) + ":"
	...
	public void precomputeLambda(State s)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s + ": evidence= " +  s.evidence + ": lambda =  " + s.lambda);
	}
	...
	void setDiscounts(History h)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(s + ": " + evidenceOf(s));
	...
	public static AbsoluteDiscountModel readFromFile(String filename)
	{
		try
		{
			...
			//t.checkConnections();
			in.close();
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(" " + t.discounts[t.MODEL_ORDER]);
			return t;
		} catch (Exception e)
		{
			e.printStackTrace();
	...
	public void checkConnections()
	{
		...
		for (TransducerWithMemory t: unsmoothedModels)
		{
			...
			for (State s: h.allStates)
			{
				...
				if (b != null)
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("backoff" + s + " -> " + b);
				}
			}
		}
	}

			...
			StateModel<History.State>, java.io.Serializable
{
	...
	public void insertHistoriesFromGraphs(java.util.Iterator<SegmentationGraph> g)
	{
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("histories inserted, start polishing .. ");
	...
	public void cleanupProlongations()
	{
		...
		for (int i=0; i < startState.prolongations.size(); i++)
		{
			...
			if (!t.target.isFinal)
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Neen:" + t.target);
		...
		for (AlignmentSegmenter.Transition t: g.outgoingEdgesOf(at))
		{
			...
			// TODO: short words: if word shorter than model order, wordfinal final states can be short
			{
				...
				if (allStates.size() > nofBefore)
				{
					...
					{
						//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("word final " + s);
						s.setWordFinal(true); 
					}
				}
			}
 ...
 */
	...
	protected class State implements java.io.Serializable
	{
		...
		private void pruneTransitions(Vector<Transition> transitions)
		{
		  ...
		  while (li.hasNext())
		  {
		  	...
		  	if (t.p < minP)
		  	{
		  		li.remove();
		  		less += minP;
		  		// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("wappp");
		  	}
		  }
		...
		private void makeStateTransitions()
		{
			...
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("OUCH " + this);
			...
			*/
			if (this == startState)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("impossible!");
				System.exit(1);
			}
		...
		public State getDownlink()
		{
			if (downlink >= 0 && History.this.lowerOrderHistory != null)
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Downlink: " + this.getEnclosingHistory().MODEL_ORDER + "  --> " + lowerOrderHistory.MODEL_ORDER);
	...
	public static void connectModels(History lowerOrder, History higherOrder)
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("connecting!! " + lowerOrder.MODEL_ORDER + " " + higherOrder.MODEL_ORDER);
	...
	{
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("connect: " + lowerOrder + "/" + lowerOrder.getEnclosingHistory() + " -- " + 
	...
	public static void main(String[] args)
	{
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("State: " + s);
			for (Transition t: s.getStateTransitions())
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("state transition: "+ t.symbol +  " -> " + t.target);
			}
			for (int i=1; i <= 20; i++)
			{
				State n = s.advance(i);
				if (n != null)
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("\tadvance on: "+ i +  " -> " + n);
				}
			}
		}
	}

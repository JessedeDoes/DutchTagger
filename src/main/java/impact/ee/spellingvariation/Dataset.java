...
public class Dataset implements Iterable<Dataitem>
{
	...
	public int read_from_file(String filename)
	{
		try
		{
			...
			while ((s = f.readLine()) != null) 
			{
				...
				if (t1.length < 3)
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("DATA ERROR: " + s);
					continue;
				}
		...
		} catch (Exception e)
		{
			...
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Unable to read file " + filename);
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Current directory: " + currentDirectory);
			e.printStackTrace();
			return 1;
		}
	}
	...
	class SegmentationGraphIterator implements Iterator<SegmentationGraph> 
	{
		...
		public void remove()
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("remove for this class not implemented");
			// does nothing!
		}
	}

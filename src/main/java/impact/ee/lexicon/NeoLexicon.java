...
public class NeoLexicon implements ILexicon,   Iterable<WordForm>
{
	...
	public void close()
	{
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Yes!!! finalize is called. Great!");
	...
	public void destroy()
	{
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Shutting down " + DB_PATH);
 ...
 */
	public void readWordsFromFile(String fileName)
	{
		...
		try
		{
			...
			while ( (s = b.readLine()) != null) // volgorde: type lemma pos lemma_pos /// why no ID's? it is better to keep them
			{
				...
				if (nItems % 50000 == 0)
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("new transaction... " + nItems);
					tx.success();
					tx.finish();
					tx =  graphDb.beginTx();
				}
	...
	public void slurpDB(String dbName)
	{
		...
		try
		{
			...
			for (WordForm w:l)
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(nItems + ": " + w);
				addWordform(w);
				nItems++;
				if (nItems % 50000 == 0)
				{
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("new transaction...");
					tx.success();
					tx.finish();
		...
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("items added: "  + nItems);
		//graphDb.shutdown();
	}
	...
	public void dumpDB()
	{
		for (Node n: graphDb.getAllNodes())
		{
			try
			{
				...
				// org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(wf);
				if (wf != null)
				{
	...
	{
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("start query for:" + lemmaQuery);
		...
		for (Node n: hits)
		{
			...
			for (Relationship r: n.getRelationships())
			{
				...
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(wordform + "\t" + lemma);
				nodes.add(wordformNode);
			}
		}
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("found items: "  + nodes.size());
		return nodes;
	}
	...
	public Set<WordForm> searchByModernWordform(String wordform) 
	{
		...
		//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("found items: "  + wordforms.size());
		return wordforms;
	}
	...
	public WordForm getWordFormFromNode(Node x)
	{
		try
		{
			...
			{
				//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("Boe! Non wordform node retrieved by wordform query?? : " + 
		...
		public boolean hasNext()
		{
			...
			while (true)
			{
				if (nodeIterator.hasNext())
				{
					...
					//org.ivdnt.openconvert.log.ConverterLog.defaultLog.println(currentNode);

...
public class Trie implements java.io.Serializable
{
	...
	public class Transition implements Serializable
	{
		...
		private void readObject(ObjectInputStream in) throws java.io.IOException, ClassNotFoundException
		{
			...
			{
				org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("hola " + allStates + " " + targetIndex);
				e.printStackTrace();
				System.exit(1);
			}
		}
	...
	public void loadWordlist(String filename, boolean assumeOrdered, boolean addWordBoundaries)
	{
		...
		try
		{
			...
			while((word = wordlist.readLine())  != null)
			{
				...
				if (nWords % 10000 == 0)
					org.ivdnt.openconvert.log.ConverterLog.defaultLog.printf("%s\n",word);
				if (assumeOrdered)
		...
		{
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("could not load word list from " + filename); 
			e.printStackTrace();
		}

		this.root.reset();
		this.root.number(0);
		org.ivdnt.openconvert.log.ConverterLog.defaultLog.printf("this trie has %d nodes and %d words\n", 
   ...
   in the suffix set. seems not much good (?) lots of clustering. 
	...
	public static void main(String[] args)
	{
		...
		try
		{
			...
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("saved...");
			t = Trie.readFromFile("/tmp/trie.out");
			org.ivdnt.openconvert.log.ConverterLog.defaultLog.println("restored...");
		} catch (Exception e)
		{
			e.printStackTrace();
		}
